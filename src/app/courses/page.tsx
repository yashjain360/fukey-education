...
import { Search, Heart, ArrowRight, Check, Filter, SlidersHorizontal, BookOpen } from "lucide-react";
...
function CoursesContent() {
  ...
  }, [selectedClasses, selectedLanguage, selectedSubject, searchQuery, sortBy]);
        ...
        <div className="rounded-3xl bg-gradient-to-r from-indigo-900 via-[#050071] to-[#1C1A4A] text-white p-8 shadow-xl relative overflow-hidden">
          <div className="relative z-10 space-y-2">
            ...
            <p className="text-xs sm:text-sm text-slate-300 max-w-xl">
              Explore 46+ comprehensive CBSE and State Board courses for Classes 9th, 10th, 11th &amp; 12th.
            </p>
          </div>
        </div>
      </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredCourses.map((course) => {
                  ...
                  return (
                    <div
                      key={course.id}
                      className="bg-white rounded-3xl border border-slate-200/80 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between overflow-hidden group"
                    >
                      {/* Course Graphic Banner */}
                      <div className="relative h-44 bg-gradient-to-br from-[#050071] via-[#1C1A4A] to-[#5751E1] p-4 flex flex-col justify-between overflow-hidden">
                        <div className="flex items-center justify-between relative z-10">
                          <span className="px-2.5 py-1 rounded-lg bg-white/20 backdrop-blur-md text-white font-black text-[10px] uppercase tracking-wider">
                            {course.class}
                          </span>
                          <button
                        ...
                        <div className="text-center relative z-10">
                          ...
                          <div className="text-[11px] text-orange-300 font-bold">
                            {course.class} • {course.language} Medium
                          </div>
                        </div>
                      ...
                      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                        <div className="space-y-2">
                          ...
                          <Link href={`/course/${course.slug}`}>
                            <h3 className="font-bold text-slate-900 text-sm group-hover:text-[#5751E1] transition-colors line-clamp-2">
                              {course.title}
                            </h3>
                          </Link>
                          ...
                          <button
                            ...
                            className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${
                              inCart
                                ...
                                : "bg-[#FF2424] hover:bg-red-700 text-white shadow-md shadow-red-500/20 active:scale-95"
                            }`}
                          >
                            {inCart ? (
                              <>
