  ...
  return (
    <section className="py-14 sm:py-20 bg-white" data-aos="fade-up">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-8 sm:space-y-12">
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto space-y-2.5 sm:space-y-3" data-aos="fade-up">
          <div className="inline-flex items-center gap-1.5 px-3.5 sm:px-4 py-1 rounded-full bg-emerald-100 text-emerald-800 font-extrabold text-[11px] sm:text-xs uppercase tracking-wider">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-stretch">
          ...
          <div
            className="lg:col-span-7 bg-gradient-to-br from-[#050071] to-[#1C1A4A] text-white rounded-3xl p-5 sm:p-8 shadow-xl flex flex-col justify-between space-y-5 sm:space-y-6"
            data-aos="fade-right"
          >
            <div className="space-y-3 sm:space-y-4">
              <div className="flex items-center justify-between border-b border-white/15 pb-3 sm:pb-4">
                ...
                <span className="px-2.5 sm:px-3 py-1 rounded-full bg-white/10 text-[11px] sm:text-xs font-black text-indigo-200">
                ...
                {(language === "hi" ? currentQ.optionsHi : currentQ.optionsEn).map((opt, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-3 sm:p-3.5 rounded-2xl text-xs sm:text-sm font-semibold flex items-center justify-between transition-all hover:scale-101 active:scale-98 cursor-pointer ${btnStyle}`}
                    >
                      <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 pr-2">
                        <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-black/20 flex items-center justify-center text-[10px] sm:text-xs font-black flex-shrink-0">
                          {String.fromCharCode(65 + idx)}
                        </span>
                        <span className="leading-snug">{opt}</span>
                      </div>

                      {isAnswered && isCorrect && <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-white flex-shrink-0" />}
                      {isAnswered && isSelected && !isCorrect && <XCircle className="w-4 h-4 sm:w-5 sm:h-5 text-white flex-shrink-0" />}
                    </button>
                  );
                })}
              </div>
              ...
              {/* Explanation Box */}
              {isAnswered && (
                <div className="p-3.5 sm:p-4 rounded-2xl bg-white/10 border border-white/15 text-xs text-indigo-100 space-y-1 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="pt-3 sm:pt-4 border-t border-white/15 flex items-center justify-between">
              ...
              <button
                ...
                className="px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl bg-[#FF2424] hover:bg-red-700 text-white font-extrabold text-xs shadow-md flex items-center gap-1.5 transition-all hover:scale-105 active:scale-95 cursor-pointer"
              >
                <span>
          ...
          <div className="lg:col-span-5 space-y-5 sm:space-y-6 flex flex-col justify-between" data-aos="fade-left">
            {/* The 45+15 Model Card */}
            <div className="bg-slate-50 rounded-3xl p-5 sm:p-6 border border-slate-200 shadow-sm space-y-4">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-indigo-100 text-[#050071] flex items-center justify-center flex-shrink-0">
                  <Clock className="w-5 h-5 sm:w-6 sm:h-6 animate-icon-wiggle" />
                </div>
                <div>
                  <h4 className="text-sm sm:text-base font-black text-slate-900">{t("hub.model_title", "The Signature 45+15 Learning Model")}</h4>
                  <p className="text-[11px] sm:text-xs text-slate-500">{t("hub.model_sub", "Live digital tablet teaching vs recorded videos")}</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 text-xs">
                <div className="p-3 sm:p-3.5 rounded-2xl bg-white border border-slate-200">
                  <div className="text-base sm:text-lg font-black text-[#5751E1]">{t("hub.lecture_time", "45 Mins")}</div>
                ...
                <div className="p-3 sm:p-3.5 rounded-2xl bg-white border border-slate-200">
                  <div className="text-base sm:text-lg font-black text-emerald-600">{t("hub.doubt_time", "15 Mins")}</div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50/80 rounded-3xl p-5 sm:p-6 border border-indigo-100 shadow-sm space-y-3 sm:space-y-4">
              <div className="flex items-center justify-between">
                ...
                <BookOpen className="w-6 h-6 sm:w-7 sm:h-7 text-[#050071] flex-shrink-0 animate-icon-float" />
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-1">
                <button
                  ...
                  className="w-full sm:w-auto px-3.5 py-2.5 rounded-xl bg-white hover:bg-indigo-50 text-[#050071] border border-indigo-200 font-bold text-xs flex items-center justify-center gap-1.5 shadow-xs transition-all hover:scale-105 active:scale-95 cursor-pointer text-center"
                >
